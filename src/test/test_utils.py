...
class TestBitboardUtils(unittest.TestCase):
    ...
    def test_get_file_from_bb(self):
        ...
        self.assertEqual(result, chess.BB_FILE_A)

    def test_get_rank_from_bb(self):
        bb = chess.BB_A1
        result = get_rank_from_bb(bb)
        self.assertEqual(result, chess.BB_RANK_1)
    ...
    def test_get_bb_diagonals_from_square_int(self):
        # Assuming the function and data in SQUARE_XRAYS are correct
        square = 0  # A1
        result = get_bb_diagonals_from_square_int(square)
        self.assertIsInstance(result, int)  # Assuming it returns an integer bitboard
