...
class Bishop(Piece):
    ...
    def get_moves(self, state):
        ...
        for current_piece_bb in get_individual_ones_in_bb(self.bb):
            ...
            bishop_actions += Action.generate_actions(destination_squares & ~state.opponent_occupied, chess.BISHOP, current_piece_position, state.turn, ActionType.MOVE)
            attack_actions += Action.generate_actions(destination_squares & state.opponent_occupied, chess.BISHOP, current_piece_position, state.turn, ActionType.ATTACK)
